...
func handleGetConfig(c *fiber.Ctx) error {
	...
	w := resp["widgets"].(fiber.Map)
	if cfg.Widgets.UniFi != nil {
		w["unifi"] = fiber.Map{
			"configured": true,
			"url":        cfg.Widgets.UniFi.URL,
		}
	} else {
		w["unifi"] = fiber.Map{"configured": false}
	}
	return c.JSON(resp)
}
...
func handleSaveConfig(c *fiber.Ctx) error {
	...
	if body.Widgets.UniFi != nil {
		if body.Widgets.UniFi.URL == "" || body.Widgets.UniFi.Username == "" || body.Widgets.UniFi.Password == "" {
			return c.Status(400).JSON(fiber.Map{"error": "unifi: url, username, and password required"})
		}
		cfg.Widgets.UniFi = &UniFiConfig{
			URL:      body.Widgets.UniFi.URL,
			Username: body.Widgets.UniFi.Username,
			Password: body.Widgets.UniFi.Password,
		}
	}
...
func handleDeleteUniFiConfig(c *fiber.Ctx) error {
	cfg := getConfig()
	cfg.Widgets.UniFi = nil
	if err := saveConfig(cfg); err != nil {
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
	return c.JSON(fiber.Map{"status": "ok"})
...
func handleUniFiClients(c *fiber.Ctx) error {
	cfg := getConfig()
	if cfg.Widgets.UniFi == nil {
		return c.Status(400).JSON(fiber.Map{"error": "unifi not configured"})
	}

	uc := newUnifiClient(cfg.Widgets.UniFi)
	if err := uc.login(); err != nil {
		return c.Status(502).JSON(fiber.Map{"error": "unifi login: " + err.Error()})
	}

	clients, err := uc.getActiveClients()
	if err != nil {
...
func handleUniFiHealth(c *fiber.Ctx) error {
	cfg := getConfig()
	if cfg.Widgets.UniFi == nil {
		return c.Status(400).JSON(fiber.Map{"error": "unifi not configured"})
	}
