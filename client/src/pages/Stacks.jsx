...
import { Layers, Plus, Trash2, Play, Terminal, X } from 'lucide-react'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {stacks.map((stack) => (
            ...
            >
              ...
              <div className="pt-2 border-t border-surface-800/50 flex justify-end">
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-2xl mx-4 rounded-xl border border-surface-700 bg-surface-900 shadow-2xl animate-slide-up">
            <div className="flex items-center justify-between p-5 border-b border-surface-800">
              ...
              <button
                onClick={() => { setShowDeploy(false); setError('') }}
            ...
            <form onSubmit={handleDeploy} className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={12}
                  className="w-full px-3 py-2 text-sm rounded-lg border border-surface-700 bg-surface-850 text-surface-200 placeholder-surface-500 focus:outline-none focus:border-accent-500/50 focus:ring-1 focus:ring-accent-500/20 transition-all font-mono resize-none"
                  required
                />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => { setShowDeploy(false); setError('') }}
                  className="px-4 py-2 text-sm font-medium rounded-lg text-surface-400 hover:text-surface-200 hover:bg-surface-800 transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={deploying}
                  className="px-4 py-2 text-sm font-medium rounded-lg bg-accent-500 hover:bg-accent-600 text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
